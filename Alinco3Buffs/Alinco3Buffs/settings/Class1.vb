...
#End Region
    ...
    Overrides Function ToString() As String
        Dim buf As String
        If (ns > 0) Then
            buf = ns.ToString("0.00", Util.NumberFormatInfo) & "N, "
        Else

            buf = Math.Abs(ns).ToString("0.00", Util.NumberFormatInfo) & "S, "
        End If

        If (ew > 0) Then
            buf = buf & ew.ToString("0.00", Util.NumberFormatInfo) & "E"
        Else

            buf = buf & Math.Abs(ew).ToString("0.00", Util.NumberFormatInfo) & "W"
        End If
    ...
    Overloads Function ToString(ByVal flag As Boolean) As String
        Dim buf As String = Me.ToString
        If flag Then
            Return "(" & "<Tell:IIDString:" & GOARROWLINK_ID & ":" & buf + ">" & buf & "<\Tell>" & ") "
        Else
            Return buf
        End If
    End Function
